...
void main() {
	...
	//如果相机高度超出大气预计算的缓冲区范围，则会出现渲染伪影
	worldPosAndCamHeight.xyz = (u_Model * vec4(a_Position, 0.0)).xyz;
	worldPosAndCamHeight.w = max(worldPosAndCamHeight.y*_uSkyAltitudeScale + _uSkyGroundOffset, 0.0);// no lower than sealevel
	MiePhase_g = PhaseFunctionG(_uSkyMieG,_uSkyMieScale);
	...
	scale = 4e-3 ;
		#endif
		Sun_g = PhaseFunctionG(.99 , SunDirectionAndSize().w * scale * _uSkyExposure);
	#else
		Sun_g = vec3(0.,0.,0.);
	#endif

	// vec4 _MoonDirSize =MoonDirectionAndSize ();
	// night sky
	vec3 right = cross( _MoonDirSize.xyz, vec3( 0., 0., 1. ));
	right = normalize(right);
	vec3 up = cross( _MoonDirSize.xyz, right );
	moonTC = vec2( dot( right, a_Position.xyz), dot( up, a_Position.xyz) )*_MoonDirSize.w + vec2(0.5);
	spaceTC = (_SpaceRotationMatrix *u_Model *  vec4(a_Position, 0.0)).xyz;//(mat3)
