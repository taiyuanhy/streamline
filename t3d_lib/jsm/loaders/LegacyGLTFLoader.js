...
var LegacyGLTFLoader = (function() {
	...
	var LegacyGLTFLoader = function(manager) {
		this.manager = (manager !== undefined) ? manager : DefaultLoadingManager;
		this.dracoLoader = null;
	}

	LegacyGLTFLoader.prototype.setDRACOLoader = function (dracoLoader) {
		this.dracoLoader = dracoLoader;
		return this;
	}

	LegacyGLTFLoader.prototype.load = function(url, onLoad, onProgress, onError) {
		var that = this;
	...
	LegacyGLTFLoader.prototype.parse = function(data, path, onLoad, onError) {
		var content;
		var extensions = {};
		...
		if (json.asset === undefined || json.asset.version[0] < 2) {
			if (onError) onError(new Error('LegacyGLTFLoader: Unsupported asset. glTF versions >=2.0 are supported.'));
			return;
		}
					...
					for (var j = 0, jl = skinEntry.joints.length; j < jl; j++) {
						...
						if (jointNode) {
							...
							if (skinEntry.inverseBindMatrices !== undefined) {
								mat.fromArray(skinEntry.inverseBindMatrices.buffer.array, j * 16);
							}

							boneInverses.push(mat);
						} else {
							console.warn('LegacyGLTFLoader: Joint "%s" could not be found.', jointId);
						}
					}

					mesh.bind(new Skeleton(bones, boneInverses), mesh.worldMatrix);
				}
			}
	...
	GLTFParser.prototype.loadCamera = function(cameraIndex) {
		...
		if (!params) {
			console.warn('t3d.LegacyGLTFLoader: Missing camera parameters.');
			return;
		}
			...
			for (var i = 0, il = animationDef.channels.length; i < il; i++) {
				...
				if (sampler) {
					...
					if (node) {
						...
						var input = new inputAccessor.buffer.array.constructor(inputAccessor.buffer.array.subarray(0, inputAccessor.buffer.array.length));
						var output = new outputAccessor.buffer.array.constructor(outputAccessor.buffer.array.subarray(0, outputAccessor.buffer.array.length));
		...
		return this.getMultiDependencies([
			...
			return scope.loadGeometries(primitives).then(function(geometries) {
				for (var i = 0, il = primitives.length; i < il; i++) {
					...
					// 	console.log('LegacyGLTFLoader: Duplicating UVs to support aoMap.');
					// 	geometry.addAttribute('a_Uv2', new Attribute(new Buffer(geometry.attributes.a_Uv.buffer.array, 2)));
					// }
					...
					if (primitive.mode === WEBGL_CONSTANTS.TRIANGLES ||
						...
						throw new Error('LegacyGLTFLoader: Primitive mode unsupported: ' + primitive.mode);
					}
     ...
     * @param  {Array<Attribute>} accessors
     */
	function addPrimitiveAttributes(geometry, primitiveDef, accessors) {
		...
		for (var gltfAttributeName in attributes) {
			...
			var attribute = accessors[attributes[gltfAttributeName]];

			// Skip attributes already provided by e.g. Draco extension.
			if (!threeAttributeName) continue;
			if (threeAttributeName in geometry.attributes) continue;

			geometry.addAttribute(threeAttributeName, attribute);
		}
	...
	GLTFParser.prototype.loadAccessor = function(accessorIndex) {
		...
		return Promise.all(pendingBufferViews).then(function(bufferViews) {
			...
			if (byteStride && byteStride !== itemBytes) {
				var ibCacheKey = 'Buffer:' + accessorDef.bufferView + ':' + accessorDef.componentType;
			...
			if (accessorDef.sparse !== undefined) {
				...
				for (var i = 0, il = sparseIndices.length; i < il; i++) {
					...
					if (itemSize >= 5) throw new Error('t3d.LegacyGLTFLoader: Unsupported itemSize in sparse Attribute.');
				}
			}
	...
	GLTFParser.prototype.loadBuffer = function(bufferIndex) {
		...
		if (bufferDef.type && bufferDef.type !== 'arraybuffer') {
			throw new Error('t3d.LegacyGLTFLoader: ' + bufferDef.type + ' buffer type is not supported.');
		}
		...
		return new Promise(function(resolve, reject) {
			loader.load(resolveURL(bufferDef.uri, options.path), resolve, undefined, function() {
				reject(new Error('t3d.LegacyGLTFLoader: Failed to load buffer "' + bufferDef.uri + '".'));
			});
		});
	};
	...
	GLTFParser.prototype.loadMaterial = function(materialIndex) {
		...
		if (materialDef.normalTexture !== undefined) {
			...
			// materialParams.normalScale = new t3d.Vector2( 1, 1 );
			//
     ...
     * @return {Promise<t3d.TextureBase>}
     */
	GLTFParser.prototype.loadTexture = function(textureIndex) {
		...
		return Promise.resolve(sourceURI).then(function(sourceURI) {
			...
			if (textureDef.internalFormat !== undefined && texture.format !== WEBGL_TEXTURE_FORMATS[textureDef.internalFormat]) {
				console.warn('t3d.LegacyGLTFLoader: js does not support texture internalFormat which is different from texture format. ' +
		...
		for (var lightId in lights) {
			...
			if (lightNode) {
				...
				if (light.fallOffExponent !== undefined) {
					console.warn('LegacyGLTFLoader:: light.fallOffExponent not currently supported.');
				}
	...
	function GLTFBinaryExtension(data) {
		...
		if (this.header.magic !== BINARY_EXTENSION_HEADER_MAGIC) {
			throw new Error('LegacyGLTFLoader: Unsupported glTF-Binary header.');
		} else if (this.header.version < 2.0) {
			throw new Error('LegacyGLTFLoader: Legacy binary file detected.');
		}
		...
		if (this.content === null) {
			throw new Error('LegacyGLTFLoader: JSON content not found.');
		}
	}
	...
	function GLTFDracoMeshCompressionExtension(dracoLoader) {
		if (!dracoLoader) {
			throw new Error('LegacyGLTFLoader: No DRACOLoader instance provided.');
		}
	...
	GLTFTextureTransformExtension.prototype.extendTexture = function (texture, transform) {
		...
		// 	console.warn('t3d.LegacyGLTFLoader: Custom UV sets in "' + this.name + '" extension not yet supported.');
		// }
