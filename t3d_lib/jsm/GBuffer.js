	...
	Object.assign(GBuffer.prototype, {
		...
		update: function(renderer, scene, camera) {
			...
			if (renderPass.capabilities.version >= 2 || renderPass.capabilities.getExtension('WEBGL_draw_buffers')) {
				...
				renderPass.renderTarget.setRenderTarget(this._renderTarget1);

				renderPass.state.colorBuffer.setClear(0, 0, 0, 0);
				...
				if (this.enableAlbedoMetalness) {
					renderPass.renderTarget.setRenderTarget(this._renderTarget2);
			...
			if (this.enableMotion) {
				renderPass.renderTarget.setRenderTarget(this._renderTarget3);
		...
		renderDebug: function(renderer, camera, type) {
			...
			this._debugPass.uniforms["viewWidth"] = renderPass.state.currentRenderTarget.width;
			this._debugPass.uniforms["viewHeight"] = renderPass.state.currentRenderTarget.height;
